...
been added to \thp{}. Conditions had to taken into account which were not
...
otherwise the standard browser right-click menu would be shown in addition to
our custom right-click menu. Another example is when a user's mouse leaves a
right-click menu, all nested menus are closed but the root menu remains open, as
depicted below.
