...
# Fork and success.
We had to fork the \thp{} repository and expose the necessary functions that
allow us to write W. The term "fork" means a copy of a repository. With the fork
of \thp{} we can now write the function W to set ~webkitdirectory~ on a file
selector, turning it into a directory selector. Figure [[fig:thp-workspace-dir]]
shows a directory selector in use in our \thp{} implementation of \utp{}, note
...
# Abstract GUI layer.
The first attempt at building a GUI for \utp{} using thp{} involved the notion
...
have the GUI only written once, GUI library agnostic, then we could plug in
