...
not provide such a function for the ~webkitdirectory~ attribute.

# Need to fork.
We might consider to write a function to set the ~webkitdirectory~ attribute
ourselves. However \thp{} does not expose the necessary functions to do so, so
we forked the \thp{} repository and exposed them. The term "fork" refers to a
copy of a repository. With the edited fork of \thp{} we can now write a function
for setting the ~webkitdirectory~ attribute on a file selector, turning it into
a directory selector. Figure [[fig:thp-workspace-dir]] shows a directory selector in
use, note that files are grayed out.
...
to reflect changes in their dependant data can be done in two ways. First, on
change of the data we can update each element which uses that data, this is the
imperative style of doing things. Second, we can define any element which uses
the data to *react* to changes in the data.
...
the element and the code handling change of the data may be in separate modules,
or at least separate to some degree. This separation is a violation of the Law
of Demeter, cite:hunt2000pragmatic, which says we should minimize coupling
between modules.

In the reactive style, when we write an element that depends on changing data we
still write code to update that element whenever the data changes. However, the
code to update the element is localised around the code where the element is
written. No longer do we have to worry about all the elements in our codebase
which depend on said data, or worry about getting the references to these
elements. We are no longer violating the Law of Demeter.

*** tododile FRP in \utp{}

** tododile Abstract GUI Layer

** tododile Integrating with Existing \utp{}
   
** tododile Leveraging Webdev Libraries
