...
# Implementation.
This paragraph relates closely to Figure [[fig:indirect-frp]]. In each of these
cases where we use FRP we have a variable, accessible application-wide, which
...
GUI library, allowing us to only write the GUI once.

# Typeclass.
The set of generic GUI functions were written as an interface, meaning the
functions did not have an implementation, only their type signatures are
declared. This interface could be implemented for each GUI library we want our
\utp{} application to support. Our \utp{} application could be written using the
generic functions of the interface, which GUI library implementation is actually
used would depend solely on which implementation we pass in as an argument to
our application.
...
problems of traditional GUI libraries. Strong future. Mention leveraging webdev
libraries.
...
|    167 | Suggested a new design for event data returned from the browser to the server. |
|    170 | Functions to write our own attribute setting functions are not exposed.        |
|    171 | "valuChange" event is not fired until user presses ESC.                        |
