...
these web technologies meaning that any layout in HTML and CSS is also possible
in \thp{}. According to the \thp{} homepage "This is a blessing, but it can also
be a curse, so the library includes a few layout combinators to quickly create
user interfaces without the need to deal with the mess that is CSS".
...
responsive layouts for \utp{} and it was a direction for \thp{} that Apfelmus
was in favour of. For these reasons we decided to write a library that would add
Flexbox support to \thp{}.
...
#+CAPTION: HTML for [[image:flex-ratio]].
#+BEGIN_SRC html
...
# #+LABEL: image:flex-ratio

#+CAPTION: Three elements in ratio.
#+LABEL: image:flex-ratio
[[./images/flex-ratio.png]]

# threepenny-gui-flexbox
We wrote a library called \tgf{} which provides a method of writing the
...
# that is required to achieve the layout in [[image:flex-ratio]] but using \thp{} and
\tgf{} instead of HTML is shown below, again note that some additional styling
code is not shown.
