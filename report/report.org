...
inheritance as can be seen in Figure [[fig:wxbutton]]. Because wxHaskell is a
wrapper around wxWidgets some concepts from wxWidgets appear in wxHaskell, in
the case of a button its type in wxHaskell is ~Window (CControl (CButton ()))~
which encodes some of the inheritance relationship.

#+CAPTION: Inheritance diagram for wxButton, cite:wxButtonClassReference.
#+NAME: fig:wxbutton
[[./images/wxbutton.png]]
...
conditions, however, now library-a might require library-d 0.5 and library-c
might require library-d > 0.6, another conflict. This endless cycle of fixing
dependency conflicts is commonly referred to as Cabal hell.
...
discussed in the previous section 2.2. If we can't build the project we can't
package a standalone application.
...
but rely on another Haskell library, such as wxHaskell, to provide the necessary
GUI bindings.
...
theory graph. Figure [[fig:thp-utp-home]] shows a single-node theory graph in our
\thp{} implementation of \utp{}, the single node is labeled "\_ROOT$0". The
screenshot also shows the current workspace being displayed.
...
chance of eventually becoming a mature and popular Haskell GUI library, which
the Haskell GUI space is in need of. For these reasons we remain highly positive
about \thp{}'s future.
...
distinguish the code it is necessary to view commit history or use GitHub's
blame tool. These repositories have also been saved to the provided CDs.

The \thp{} implementation of \utp{} produced can be found on GitHub at:

https://github.com/andrewbutterfield/utp2/tree/feature/threepenny-gui

Interactions with the \thp{} repository, including pull requests and issues can
be found on GitHub at:

https://github.com/HeinrichApfelmus/threepenny-gui/issues?q=author%3Abarischj

The \tgc{} library (right-click menus for \thp{}) produced can be found on
GitHub at:

https://github.com/barischj/threepenny-gui-contextmenu

The \tgf{} library (Flexbox combinators for \thp{}) produced can be found on
GitHub at:
