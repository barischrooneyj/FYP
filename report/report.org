...
line we attach that button to the HTML ~<body>~. The third line causes its body
to be evaluated when a user clicks the button. The fourth line is what is then
evaluated, it changes the button's text to "I have been clicked!".
...
a right-click menu using \thp{} represented, to some degree, an investigation
into the feasibility of using \thp{} to build an entire GUI for \utp{}. This is
because a right-click menu is one of the more complex components of a GUI and
also because of the widespread use of right-click menus in \utp{}.
...
\thp{} does not provide a facility to build a right-click menu. You might expect
that a GUI library would provide support for this, since it seems like one of
the fundamental parts of a GUI. However \thp{}'s approach is different to a
traditional GUI library, it acts as a wrapper around existing web technologies,
leveraging their power. This means that building a right-click menu in \thp{}
instead becomes the problem of building a right-click menu using web
technologies.
...
right-click menu however it is only implemented and enabled by Mozilla's Firefox
...
does not support the specification. The HTML specification for building a
...
While most browser's do not have enabled support for right-click menus based on
the HTML specification, all major browsers have support for the contextmenu
event, which can be used to build a right-click menu.

# contextmenu event.
HTML consists of a tree of elements such as ~<body>~, ~<div>~ or ~<button>~, an
example of HTML's tree structure can be seen in the image below. When a
JavaScript event occurs at one of these elements it propagates upward through
the tree of elements. An example of an event occurring at an element is a
"click" event occurring when a user clicks on an element, the "click" event
propagates upward through the tree of elements unless it is handled or it
reaches the root element, this idea of events propagating and being handled is
very similar to the idea of exceptions propagating and being captured. The event
we are interested in is the "contextmenu" event.

# Preventing propagation.

# Modifying Threepenny-gui.
