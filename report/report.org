...
A goal of Andrew Butterfield's (TODO cite) while developing \utp{} was to reach
a point where standalone applications of \utp{} could be distributed e.g. ~.deb~
packages for Debian or ~.app~ bundles for macOS, or if not native applications
then at least executables. This proved difficult for the existing project as it
was not being successfully built on macOS and was difficult to build on Linux,
however executables for Windows do exist and are hosted on the project's
homepage. At least on macOS the difficulties in building the project are largely
related to wxHaskell, for reasons discussed in the previous section 2.2.
...
the element and the code handling a change in the data may be in separate
...
simply declare that it should have the /current/ value of the data, this data is
held in some variable D. Now the code handling a change in the data, namely
Listing [[list:frp-change-data]] simply updates D, any elements which depend on D
will be updated automatically, see Figure [[fig:indirect-frp]]. Notably the code
handling a change to the data does not have to be concerned with elements
depending on the data, we are no longer violating the Law of Demeter.
...
# Summary.
To summarise, functional reactive programming is preferable because when some
data changes we don't have to worry about updating the elements that depend on
the data directly. Instead we update the elements indirectly, by emitting a new
value of the data which the elements then receive. By not updating the elements
directly we are not violating the Law of Demeter.

#+CAPTION: With FRP we don't have to worry about updating element state.
...
executed once the user provides input. This isn't really \thp{}'s fault, it is
...
# Benefits.
\thp{} has benefits over other Haskell GUI libraries, benefits that primarily
stem from \thp{}'s use of the web browser as a display. \thp{} is a standalone
GUI library; in contrast with wxHaskell which depends on another GUI library,
and not even a haskell library, but a C++ library. Another advantage of \thp{}
using the web browser as a display is that we can leverage come of the many web
development libraries that exist, such as MaterializeCSS which we used to build
the white workspace selection prompt shown in Figure [[fig:thp-workspace-prompt]].
Other benefits of \thp{} include: high level abstractions out-of-the box, it is
easy to install, and if you have familiarity with web technologies then it has
quite a gentle learning curve.

# Disadvantages.
However \thp{} is a young library, and as we discovered, some things which might
be easy to accomplish in other GUI libraries were difficult with \thp{}. \thp{}
does not provide support for building a right-click menu or a file selector.
Building a right-click menu was problematic because web browsers have not yet
implemented the respective specification, and implementing file selection was
tough because of a browser security feature that obfuscates file paths. The
difficulty in implementing these features stemmed from \thp{} using the web
browser as a display. So, while \thp{} using the web browser as a display does
provide the library with benefits it also presents issues.

# Young library, small issues, maintainable.
It would be unusual for a young library to have issues. Importantly these issues
are small enough to be overcome, as we did for the issues discovered in our
project. Key to being able to address the issues was the moderately small size
of \thp{}'s code base, the small size made it easy enough to understand \thp{}'s
code, at least to the extent where we could write our own fixes when needed.
\thp{}'s small size is essential for its maintainability, and if we are able to
get involved and address issues then so can others. This low maintenance cost
and the ability of user's to get involved improves \thp{}'s chances of survival
and of growth in the jungle that is the Haskell GUI space, and is why we remain
highly positive about \thp{}'s future.
...
** TODO threepenny-gui-contextmenu
